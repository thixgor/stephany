...
import { FiArrowLeft, FiSave, FiPlus, FiTrash2, FiActivity, FiFileText, FiPlusCircle } from 'react-icons/fi';
import SignaturePad from '@/components/ui/SignaturePad';
import { getClinicalConstants, isOutOfRange } from '@/lib/clinicalConstants';

interface Appointment {
    _id: string;
    patientId: { _id: string; name: string; tutorName: string; species: string; breed: string; tutorCpf: string; age?: string; weight?: number; gender?: string };
...
export default function NovoLaudoPage() {
    ...
    useEffect(() => {
        // Fetch recent appointments (last 50) - without status filter to ensure they appear
    ...
    useEffect(() => {
        if (formData.appointmentId && formData.type) {
            const appt = appointments.find(a => a._id === formData.appointmentId);
            if (appt) {
                const now = new Date();
                ...
                let prefix = 'Laudo Médico';
                if (formData.type === 'receita') prefix = 'Receita Veterinária';
                if (formData.type === 'exame') prefix = 'Resultado de Exame';
                if (formData.type === 'atestado') prefix = 'Atestado de Saúde';
                if (formData.type === 'encaminhamento') prefix = 'Encaminhamento';

                setFormData(prev => ({
                    ...prev,
                    patientId: appt.patientId._id,
                    title: `${prefix} - ${appt.patientId.name} - ${dateStr}`
                }));
                setCurrentSpecies(appt.patientId.species);
            }
        }
    }, [formData.appointmentId, formData.type, appointments]);
    ...
    return (
        <div className="space-y-6 max-w-5xl mx-auto pb-20">
            <div className="flex items-center gap-4">
                ...
                <div>
                    <h1 className="text-2xl font-bold text-[#00231F]">Novo Documento Profissional</h1>
                    <p className="text-gray-600">Gere laudos, receitas e atestados com verificação SHA-256 e QR Code.</p>
                </div>
            </div>
                                    ...
                                    {formData.prescription.map((p, i) => (
                                        <div key={i} className="p-4 bg-gray-50 rounded-xl border border-gray-100 flex flex-col gap-4">
                                            <div className="flex justify-between">
                                                ...
                                                <button onClick={() => setFormData({ ...formData, prescription: formData.prescription.filter((_, idx) => idx !== i) })} className="text-red-500 hover:text-red-700"><FiTrash2 /></button>
                                            </div>
